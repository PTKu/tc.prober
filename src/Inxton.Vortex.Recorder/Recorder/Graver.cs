...
namespace Inxton.Vortex.Recorder
{
    internal class Graver<T, P> : RecorderBase<T, P>, IRecorder where T : IVortexObject, new() where P : new()
    {
        ...
        public void Stop(string fileName)
        {
            ...
            {
                throw new InsufficientNumberOfFramesException($"There is no sufficient number of unique frames recoded ('{squashedRecording.Frames.LongCount()}'). Minimum required number of frames is '{MinUniqueFrames}'. " +
